...
class HAPPO():
    ...
    def train(self, buffer, update_actor=True, update_critic=True):
        ...
        if self.args.norm_type == 'pre_norm':
            if buffer.factor is None:
                pass
            else:
                advantages = advantages * buffer.factor#np.clip(buffer.factor, 1.0 - self.args.factor_clip, 1.0 + self.args.factor_clip)
        
        #print(1, buffer.factor[:5,:4,0])
        ...
        if self.args.norm_type == 'post_norm':
            if buffer.factor is None:
                pass
            else:
                advantages = advantages * buffer.factor# np.clip(buffer.factor, 1.0 - self.args.factor_clip, 1.0 + self.args.factor_clip)
        
        elif self.args.norm_type == 'double_norm':
            if buffer.factor is None:
                pass
            else:
                advantages = advantages * buffer.factor#np.clip(buffer.factor, 1.0 - self.args.factor_clip, 1.0 + self.args.factor_clip)
        ...
        #print(2,imp_weights.reshape(1000, 4)[:5,:4])
