...
from tianshou.policy import MATRPOPolicy
...
def get_args():
    ...
    parser.add_argument('--logdir', type=str, default='MATRPO')
    parser.add_argument('--render', type=float, default=0.)
    parser.add_argument('--norm-adv', type=int, default=1)
    parser.add_argument('--optim-critic-iters', type=int, default=16)
...
def test_trpo(args=get_args()):
    ...
    for i in range(args.num_agents):
        actor = ActorProb(net_a, args.action_shape[0]/args.num_agents, max_action=args.max_action,
    ...
    for i in range(args.num_agents):
        torch.nn.init.constant_(actors[i].sigma_param, -0.5)
        for m in actors[i].mu.modules():
            if isinstance(m, torch.nn.Linear):
                torch.nn.init.zeros_(m.bias)
                m.weight.data.copy_(0.01 * m.weight.data)
    for m in [actors[i].modules() for i in range(args.num_agents)] + list(critic.modules()):
        if isinstance(m, torch.nn.Linear):
    ...
    policy = MATRPOPolicy(actors, critic, optim, args.num_agents, dist, discount_factor=args.gamma,
    ...
    log_path = os.path.join(args.logdir, args.task, 'trpo', str(args.num_agents),log_file)
